...
//	private static final Logger logger = LogManager.getLogger(threadNNo1.class.getName());
	...
	public threadNNo4(Queue<Integer> queue, int missionlong, List<Map<String, Object>> mission, String name, int type) {
		...
		while (!finished) {
			synchronized (queue) {
				...
				if (missionlong == 0) {
					...
					System.out.println("threadNNo4线程执行结束,共耗时" + (endTime - startTime) + "毫秒");
					stopMe();
				}
				/////////////////////////////////////////
				ThExeMet.method(mission.get(missionlong), type, name);
				/////////////////////////////////////////
				System.out.println("threadNNo4线程执行任务:还有" + missionlong + "次执行");
				missionlong--;
			}
		}
	}
